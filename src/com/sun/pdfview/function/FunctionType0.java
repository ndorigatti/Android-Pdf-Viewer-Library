...
public class FunctionType0 extends PDFFunction {
	...
	protected void doFunction(float[] inputs, int inputOffset, float[] outputs, int outputOffset) {
		...
		for (int i = 0; i < outputs.length; i++) {
			...
			outputs[i + outputOffset] = interpolate(outputs[i + outputOffset], 0, (float) Math.pow(2, getBitsPerSample()) - 1, getDecode(2 * i),
					getDecode((2 * i) + 1));
		}
	}
	 ...
	 * 
	 * @param i
	 *            the index into the decoding array, which has size 2 * <i>n</i>.
	 *            the <i>i</i>th entry in the array has index 2<i>i</i>,
	 *            2<i>i</i> + 1
	 ...
	 * 
	 * @param values
	 *            an array of <i>m</i> values determining which sample
	 *            to select
	 * @param od
	...
	private float multicubicInterpolate(float[] encoded, int od) {
		Log.e(Constants.TAG, "Cubic interpolation not supported!");
