...
public class FunctionType3 extends PDFFunction {
	 ...
	 * <li><b>Functions</b> <i>array</i> (Required) An array of k 1-input functions making up the stitching function. The output dimensionality of all functions
	 * must be the same, and compatible with the value of <b>Range</b> if <b>Range</b> is present.</li>
	 * <li><b>Domain</b><i>array</i> (Required) A 2 element array where <b>Domain</b>0 is less than <b>Domain</b>1. This is read by the <code>PDFFunction</code>
	 * superclass.</li>
	 * <li><b>Bounds</b> <i>array</i> (Required) An array of k-1 numbers that, in combination with <b>Domain</b>, define the intervals to which each function
	 * from the <b>Functions</b> array applies. <b>Bounds</b> elements must be in order of increasing value, and each value must be within the domain defined by
	 * >b>Domain</b>.</li>
	 * <li><b>Encode</b> <i>array</i> (Required) An array of 2 * k numbers that, taken in pairs, map each subset of the domain defined by <bDomain</b> and the
	 * <b>Bounds</b> array to the domain of the corresponding function.</li>
	 * </p>
	 */
	@Override
	protected void parse(PDFObject obj) throws IOException {
		...
		if (boundsAry.length != k - 1) {
			throw new PDFParseException("Bounds array length " + boundsAry.length + " should be " + (k - 1) + " with functions array length "
					+ functions.length);
		}
