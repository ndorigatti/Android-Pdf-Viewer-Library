...
public class PDFPage
{
	...
	private final Map< ImageInfo, WeakReference > renderers = Collections
	 ...
	 * create a PDFPage with dimensions in bbox and rotation.
	 */
	public PDFPage ( int pageNumber, RectF bbox, int rotation, Cache cache )
	{
		...
		{
			bbox = new RectF( bbox.left, bbox.top, bbox.left + bbox.height(), bbox.top + bbox.width() );
		}
	 ...
	 * @param clip
	 *            the region in <b>page space</b> of the page to display. It may be null, in which the page's defined crop box will be used.
	 */
	public PointF getUnstretchedSize ( int width, int height, RectF clip )
	{
		if ( clip == null )
		{
			clip = bbox;
		...
		else
		{
			if ( rotation == 90 || rotation == 270 )
			{
				clip = new RectF( clip.left, clip.top, clip.height(), clip.width() );
			}
		}
	 ...
	 * @param clip
	 *            the region in <b>page space</b> of the entire page to display
	 * @param observer
	 *            an image observer who will be notified when the image changes, or null
	 ...
	 *            an image observer who will be notified when the image changes, or null
	 * @param drawbg
	 *            if true, put a white background on the image. If not, draw no color (alpha 0) for the background.
	 * @param wait
	 *            if true, do not return until this image is fully rendered.
	 * @return an Image that contains the PDF data
	 */
	public Bitmap getImage ( int width, int height, RectF clip, boolean drawbg, boolean wait )
	{
...
class PDFChangeStrokeCmd extends PDFCmd
{
	...
	public String toString ( PDFRenderer state )
	{
		return "STROKE: w=" + w + " cap=" + cap + " join=" + join + " limit=" + limit + " ary=" + Arrays.toString( ary )
				+ " phase=" + phase;
	}
}
