...
public class Type1Font extends OutlineFont {
	 ...
	 * <p>
	 * this function attempts to parse an array out of a postscript definition without doing any postscript. It's actually looking for things that look like
	 * "dup <i>id</i> <i>elt</i> put", and placing the <i>elt</i> at the <i>i</i>th position in the array.
	 * 
	 * @param d
	 *            the font definition stream
	 * @param key
	 ...
	 * @param end
	 *            a string that appears at the end of the array
	 ...
	 * @param start
	 *            where in the array to start decoding
	 * @param end
	 *            where in the array to stop decoding
	 * @param key
	 *            the decryption key
	 * @param skip
	 *            how many bytes to skip initially
	 ...
	 * 
	 * @param data
	 *            the data, formatted as ASCII strings
	 * @param start
	 *            where in the array to start decrypting
	 * @param end
	 ...
	 *            the search array
	 * @param name
	 *            the name to look for, without the initial /
	 ...
	 * 
	 * @param cs
	 *            the decrypted glyph data
	 * @param gp
	 *            a GeneralPath into which the glyph shape will be stored
	 * @param pt
	...
	private void parse(byte[] cs, Path gp, FlPoint pt, FlPoint wid) {
		...
		while (loc < cs.length) {
			...
			if (v == 255) {
				stack[sloc++] = ((((int) cs[loc]) & 0xff) << 24) + ((((int) cs[loc + 1]) & 0xff) << 16) + ((((int) cs[loc + 2]) & 0xff) << 8) + ((((int) cs[loc + 3]) & 0xff));
				loc += 4;
				...
				switch (v) {
					...
					case 8: // x1 y1 x2 y2 x3 y3 rcurveto
						...
						y1 = pop();
						x1 = pop();
						gp.cubicTo(pt.x + x1, pt.y + y1, pt.x + x1 + x2, pt.y + y1 + y2, pt.x + x1 + x2 + x3, pt.y + y1 + y2 + y3);
						pt.x += x1 + x2 + x3;
						pt.y += y1 + y2 + y3;
						sloc = 0;
						break;
					...
					case 30: // y1 x2 y2 x3 vhcurveto
						...
						y1 = pop();
						x1 = y3 = 0;
						gp.cubicTo(pt.x, pt.y + y1, pt.x + x2, pt.y + y1 + y2, pt.x + x2 + x3, pt.y + y1 + y2);
						pt.x += x2 + x3;
						pt.y += y1 + y2;
						sloc = 0;
						break;
					case 31: // x1 x2 y2 y3 hvcurveto
						...
						x1 = pop();
						y1 = x3 = 0;
						gp.cubicTo(pt.x + x1, pt.y, pt.x + x1 + x2, pt.y + y2, pt.x + x1 + x2, pt.y + y2 + y3);
						pt.x += x1 + x2;
						pt.y += y2 + y3;
						sloc = 0;
						break;
	 ...
	 * 
	 * @param x
	 *            the x offset of the accent relativ to the sidebearing of the base char
	 * @param y
	 *            the y offset of the accent relativ to the sidebearing of the base char
	 * @param a
	 *            the index of the accent glyph
	 * @param b
	 *            the index of the base glyph
	 * @param gp
	 *            the GeneralPath into which the combined glyph will be
	 *            written.
	 */
	private void buildAccentChar(float x, float y, char a, char b, Path gp) {
	 ...
	 */
	private synchronized Path parseGlyph(byte[] cs, FlPoint advance, Matrix at) {
		Path gp = new Path();
