...
/**
 * This class turns a set of PDF Commands from a PDF page into an image. It encapsulates the state of drawing in terms of stroke, fill, transform, etc., as well as pushing and popping these states.
 * 
 * When the run method is called, this class goes through all remaining commands in the PDF Page and draws them to its buffered image. It then updates any ImageConsumers with the drawn data.
 * 
 * @author ?
...
public class PDFRenderer extends BaseWatchable
{
	 ...
	 *            the current page
	 * @param imageinfo
	 *            the paramters of the image to render
	 */
	 ...
	 * @param imgbounds
	 *            the bounds of the image into which to fit the page
	 * @param clip
	 *            the portion of the page to draw, in page space, or null if the whole page should be drawn
	 * @param bgColor
	 *            the color to draw the background of the image, or null for no color (0 alpha value)
	 */
	public PDFRenderer ( PDFPage page, Canvas g, RectF imgbounds, RectF clip, int bgColor )
	{
		super();

		this.page = page;
		this.g = g;
		this.imageinfo = new ImageInfo( ( int ) imgbounds.width(), ( int ) imgbounds.height(), clip, bgColor );
	 ...
	 * @param s
	 *            the path to stroke
	 * @return a Rectangle2D to which the current region being drawn will be added. May also be null, in which case no dirty region will be recorded.
	 */
	...
	public RectF drawImage ( PDFImage image )
	{
		...
		Utils.setMatValues( mat, 1f / image.getWidth(), 0, 0, -1f / image.getHeight(), 0, 1 );
		...
		float[] points = new float[] { minx, miny, minx + bi.getWidth(), miny + bi.getHeight() };
		matB.mapPoints( points, 0, points, 0, 2 );

		return new RectF( points[ 0 ], points[ 1 ], points[ 2 ] - points[ 0 ], points[ 3 ] - points[ 1 ] );
	 ...
	 * 
	 * @param w
	 *            the width of the stroke, or NOWIDTH to leave it unchanged
	 * @param cap
	 *            the end cap style, or NOCAP to leave it unchanged
	 * @param join
	 *            the join style, or NOJOIN to leave it unchanged
	 * @param limit
	 *            the miter limit, or NOLIMIT to leave it unchanged
	 * @param phase
	 *            the phase of the dash array, or NOPHASE to leave it unchanged
	 * @param ary
	 *            the dash array, or null to leave it unchanged. phase and ary must both be valid, or phase must be NOPHASE while ary is null.
	 */
	 ...
	 * 
	 * @return <ul>
	 *         <li>Watchable.RUNNING when there are commands to be processed
	 *         <li>Watchable.NEEDS_DATA when there are no commands to be processed, but the page is not yet complete
	 *         <li>Watchable.COMPLETED when the page is done and all the commands have been processed
	 *         <li>Watchable.STOPPED if the image we are rendering into has gone away
	 *         </ul>
	 */
	@Override
	...
	class GraphicsState implements Cloneable
	{
		 ...
		 * 
		 * Note that cliprgn is not cloned. It must be set manually from the current graphics object's clip
		 */
		@Override
		public Object clone ()
		{
