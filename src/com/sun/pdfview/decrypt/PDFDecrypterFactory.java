...
public class PDFDecrypterFactory {
	 ...
	 *         PDF
	 * @throws IOException
	 *             will typically be a {@link com.sun.pdfview.PDFParseException}, indicating an IO problem, an error
		...
		if (encryptDict == null) {
			...
			if (filter != null && "Standard".equals(filter.getStringValue())) {
				...
				if (v == 1 || v == 2) {
					...
					final Integer length = lengthObj != null ? lengthObj.getIntValue() : null;
	 ...
	 * @throws IOException
	 *             if there is a problem reading the PDF, an invalid
	...
	private static PDFDecrypter createCryptFilterDecrypter(PDFObject encryptDict, PDFObject documentId, PDFPassword password, int v)
		...
		final Iterator<String> cfNameIt = (Iterator<String>) cfDict.getDictKeys();
		while (cfNameIt.hasNext()) {
			...
			final Integer length = lengthObj != null ? lengthObj.getIntValue() * 8 : null;
			...
			if ("None".equals(cfm)) {
				...
				cfDecrypter = createStandardDecrypter(encryptDict, documentId, password, length, encryptMetadata,
	 ...
	 *            the password
	 * @param keyLength
	 *            the key length, in bits; may be <code>null</code> to use a {@link #DEFAULT_KEY_LENGTH default}
	 * @param encryptMetadata
	 *            whether metadata is being encrypted
	 ...
	 * @throws IOException
	 *             if there is a problem reading the PDF content,
	 ...
	 *             not currently support the specified encryption
	 */
	private static PDFDecrypter createStandardDecrypter(PDFObject encryptDict, PDFObject documentId, PDFPassword password, Integer keyLength,
			boolean encryptMetadata, StandardDecrypter.EncryptionAlgorithm encryptionAlgorithm) throws PDFAuthenticationFailureException,
			IOException, EncryptionUnsupportedByPlatformException, EncryptionUnsupportedByProductException {
