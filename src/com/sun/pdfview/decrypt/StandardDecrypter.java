...
public class StandardDecrypter implements PDFDecrypter {
	 ...
	 */
	private final static byte[] PW_PADDING = new byte[] { 0x28, (byte) 0xBF, 0x4E, 0x5E, 0x4E, 0x75, (byte) 0x8A, 0x41, 0x64, 0x00, 0x4E, 0x56,
			(byte) 0xFF, (byte) 0xFA, 0x01, 0x08, 0x2E, 0x2E, 0x00, (byte) 0xB6, (byte) 0xD0, 0x68, 0x3E, (byte) 0x80, 0x2F, 0x0C, (byte) 0xA9,
			(byte) 0xFE, 0x64, 0x53, 0x69, 0x7A };
	 ...
	 * @param uValue
	 *            the value of the U entry from the Encrypt dictionary
	 * @param pValue
	 *            the value of the P entry from the Encrypt dictionary
	 ...
	 * @throws EncryptionUnsupportedByProductException
	 *             if PDFRenderer does not
	 *             currently support the specified encryption
	 */
	public StandardDecrypter(EncryptionAlgorithm encryptionAlgorithm, PDFObject documentId, int keyBitLength, int revision, byte[] oValue,
			byte[] uValue, int pValue, boolean encryptMetadata, PDFPassword password) throws IOException, EncryptionUnsupportedByProductException,
		...
		try {
			...
			for (int i = 0; generalKeyBytes == null && i < passwordBytePossibilities.size(); ++i) {
				...
				if (generalKeyBytes != null) {
					...
					generalKeyBytes = checkUserPassword(passwordBytes, firstDocIdValue, keyBitLength, revision, oValue, uValue, pValue,
							encryptMetadata);
	 ...
	 * 
	 * @param keyBitLength
	 *            the length of the content key, in bits
	 * @throws EncryptionUnsupportedByPlatformException
	 *             if the platform does not
	 *             support the required ciphers and key lengths
	 * @throws PDFParseException
	 *             if there's an internal error while testing
	 *             cipher availability
	 */
	private void testJceAvailability(int keyBitLength) throws EncryptionUnsupportedByPlatformException, PDFParseException {
		...
		try {
			...
			throw new EncryptionUnsupportedByPlatformException("JCE does accept key size of " + (getSaltedContentKeyByteLength() * 8)
	...
	private ByteBuffer decryptBuffer(ByteBuffer encrypted, byte[] decryptionKeyBytes) throws PDFParseException {
		...
		try {
			...
			final java.nio.ByteBuffer decryptedBuf = java.nio.ByteBuffer.allocate(encrypted.remaining());
	 ...
	 * 
	 * @param encrypted
	 *            the encrypted content; required by AES encryption so
	 ...
	 * @throws PDFParseException
	 *             if the encrypted buffer is malformed or on an
	 *             internal error
	 * @throws NoSuchAlgorithmException
	 *             if the cipher algorithm is not supported
	 *             by the platform
	 * @throws NoSuchPaddingException
	 *             if the cipher padding is not supported by
	 *             the platform
	 * @throws InvalidKeyException
	 *             if the key is invalid according to the
	 *             cipher, or too long
	 * @throws InvalidAlgorithmParameterException
	 *             if the cipher parameters are
	 *             bad
	 */
	private Cipher createAndInitialiseContentCipher(ByteBuffer encrypted, byte[] decryptionKeyBytes) throws PDFParseException,
	 ...
	 * 
	 * @param generalKeyByteLength
	 *            the length of the general key, in bytes
	 ...
	 *            the object generation
	 * @throws PDFParseException
	 *             if the object numbering indicates that they
	 *             aren't true object numbers
	 */
	private static void checkNums(int objNum, int objGen) throws PDFParseException {
		if (objNum < 0) {
	 ...
	 * @param firstDocIdValue
	 *            the value of the first element in the document's
	 *            ID entry in the trailer dictionary
	 * @param revision
	 *            the revision of the security handler
	 * @return the U value for the given configuration
	 * @throws GeneralSecurityException
	 *             if there's an error getting required
	 ...
	 *             supported
	 */
	private static byte[] calculateUValue(byte[] generalKey, byte[] firstDocIdValue, int revision) throws GeneralSecurityException,
			EncryptionUnsupportedByProductException {

		if (revision == 2) {

			// Algorithm 3.4: Computing the encryption dictionarys (user
			...
			// Algorithm 3.5: Computing the encryption dictionary (user
	 ...
	 * 
	 * @param ownerPassword
	 *            the suggested owner password (may be null or
	 *            empty)
	 * @param firstDocIdValue
	 *            the byte stream from the first element of the
	 ...
	 *            the security handler revision
	 * @param oValue
	 *            the O value from the Encrypt dictionary
	 * @param uValue
	 *            the U value from the Encrypt dictionary
	 * @param pValue
	 *            the P value from the Encrypt dictionary
	 * @param encryptMetadata
	 *            the EncryptMetadata entry from the Encrypt dictionary
	 *            (or false if not present or revision &lt;= 3)
	 * @return the general/user key bytes if the owner password is currect, <code>null</code> otherwise
	 * @throws GeneralSecurityException
	 *             if there's a problem with
	 *             cipher or digest usage; unexpected
	 * @throws EncryptionUnsupportedByProductException
	 *             if PDFRenderer doesn't
	 *             support the security handler revision
	 * @throws PDFParseException
	 *             if the document is malformed
	 */
	private static byte[] checkOwnerPassword(byte[] ownerPassword, byte[] firstDocIdValue, int keyBitLength, int revision, byte[] oValue,
			byte[] uValue, int pValue, boolean encryptMetadata) throws GeneralSecurityException, EncryptionUnsupportedByProductException,
			PDFParseException {
		...
		if (revision == 2) {
			...
			// dictionary entry, using an RC4 encryption function with the
			...
			// value of the encryption dictionary entry (first iteration) or
	 ...
	 */
	private static byte[] getInitialOwnerPasswordKeyBytes(byte[] ownerPassword, int keyBitLength, int revision) throws GeneralSecurityException {
	 ...
	 * 
	 * @param userPassword
	 *            the user password to test; may be null or empty
	 * @param firstDocIdValue
	 *            the byte stream from the first element of the
	 *            value of the ID entry in the trailer dictionary
	 * @param keyBitLength
	 ...
	 */
	private static byte[] checkUserPassword(byte[] userPassword, byte[] firstDocIdValue, int keyBitLength, int revision, byte[] oValue,
			byte[] uValue, int pValue, boolean encryptMetadata) throws GeneralSecurityException, EncryptionUnsupportedByProductException,
			PDFParseException {
		...
		final byte[] generalKey = calculateGeneralEncryptionKey(userPassword, firstDocIdValue, keyBitLength, revision, oValue, pValue,
				encryptMetadata);
		...
		// encryption dictionary entry (comparing on the first 16 bytes in
	 ...
	 * 
	 * @param userPassword
	 *            the desired user password; may be null or empty
	 * @param firstDocIdValue
	 *            the byte stream from the first element of the
	 *            value of the ID entry in the trailer dictionary
	 * @param keyBitLength
	 ...
	 * @throws GeneralSecurityException
	 *             if an error occurs when obtaining
	 *             and operating ciphers/digests
	 */
	private static byte[] calculateGeneralEncryptionKey(byte[] userPassword, byte[] firstDocIdValue, int keyBitLength, int revision, byte[] oValue,
			int pValue, boolean encryptMetadata) throws GeneralSecurityException {
		...
		// Step 3: Pass the value of the encryption dictionary entry to the
		...
		// Step 5: Pass the first element of the file file identifier array
		...
		// 3 or greater, depends on the value of the encryption dictionary
		// Length entry.
	 ...
	 *             if there's bad padding
	 * @throws IllegalBlockSizeException
	 *             if the block size is bad
	 */
	private static byte[] crypt(Cipher cipher, byte[] input) throws IllegalBlockSizeException, BadPaddingException {
	 ...
	 */
	private static void initEncryption(Cipher cipher, SecretKey key) throws InvalidKeyException {
	 ...
	 * 
	 * @param shuffle
	 *            the bytes to be shuffled
	 * @param key
	 *            the original key
	 * @param rc4
	 *            the cipher to use
	 * @throws GeneralSecurityException
	 *             if there's a problem with cipher
	 *             operation
	 */
	private static void rc4shuffle(byte[] shuffle, byte[] key, Cipher rc4) throws GeneralSecurityException {
	 ...
	 * 
	 * @param rc4
	 *            the RC4 cipher to use
	 * @param shuffle
	 *            the bytes in which shuffling will take place; unshuffling
	 *            happens in place
	 * @param key
	 *            the encryption key
	 * @throws GeneralSecurityException
	 ...
	 *             operation
	 */
	private static void rc4unshuffle(Cipher rc4, byte[] shuffle, byte[] key) throws GeneralSecurityException {
	 ...
	 * 
	 * @param rc4
	 *            the cipher to use; must be a stream cipher producing
	 *            identical output length to input (e.g., RC4)
	 * @param buffer
	 *            the buffer to read input from and write output to
	 * @throws IllegalBlockSizeException
	 ...
	 *             if an inappropriate cipher is used
	 */
	private static void cryptInPlace(Cipher rc4, byte[] buffer) throws IllegalBlockSizeException, ShortBufferException, BadPaddingException {
	 ...
	 *            the cipher key
	 * @throws InvalidKeyException
	 *             if the key is of an unacceptable size or
	 *             doesn't belong to the cipher
	 */
	private static void initDecryption(Cipher cipher, Key aKey) throws InvalidKeyException {
	 ...
	 * @return the cipher
	 * @throws NoSuchAlgorithmException
	 *             if the RC4 cipher is unavailable
	 * @throws NoSuchPaddingException
	 *             should not happen, as no padding
	 *             is specified
	 */
	private static Cipher createRC4Cipher() throws NoSuchAlgorithmException, NoSuchPaddingException {
	 ...
	 * @throws NoSuchAlgorithmException
	 *             if the AES cipher is unavailable
	 * @throws NoSuchPaddingException
	 *             if the required padding is unavailable
	 */
	private static Cipher createAESCipher() throws NoSuchAlgorithmException, NoSuchPaddingException {
	 ...
	 *            the MD5 digest
	 * @param hash
	 *            the hash destination
	 * @throws GeneralSecurityException
	 *             if there's a problem hashing; e.g.,
	 *             if the buffer is too small
	 */
	private static void digestTo(MessageDigest md5, byte[] hash) throws GeneralSecurityException {
